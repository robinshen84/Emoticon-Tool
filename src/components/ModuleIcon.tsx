...
export const ModuleIcon: React.FC = () => {
  ...
  // Modes: 'auto' (Auto Center), 'custom' (Manual Crop)
  ...
  const handleDownload = () => {
    if (processedBlob) {
      downloadBlob(processedBlob, `icon_50x50_${file?.name || 'image'}`);
    }
  };
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <h3 className="text-lg font-medium">表情图标 (50x50)</h3>
        {file && (
          <button 
            onClick={handleClear}
      ...
      {!file && (
        <Dropzone onDrop={handleDrop} multiple={false} text="拖拽一张图标原图到此处" />
      )}
      ...
      {file && previewUrl && (
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          ...
          <div className="space-y-4">
            ...
            <div className="flex gap-2">
              <button
                ...
                className={`flex-1 p-2 rounded-lg border-2 flex flex-col items-center gap-1 transition-all ${mode === 'auto' ? 'border-blue-500 bg-blue-50 dark:bg-blue-900/20 text-blue-600' : 'border-gray-200 dark:border-gray-700 hover:border-blue-300'}`}
              >
          ...
          <div className="space-y-2">
            <h4 className="font-medium text-sm text-gray-500">效果预览 (50x50)</h4>
            <div className="bg-gray-100 dark:bg-gray-700 rounded-lg overflow-hidden flex flex-col items-center justify-center p-4 min-h-[300px] gap-8">
              {processedUrl ? (
                <>
                  <div className="flex gap-8 items-end">
                    <div className="text-center">
                      <div className="bg-white dark:bg-gray-800 shadow-md p-1 rounded inline-block">
                        <img src={processedUrl} alt="50x50" className="w-[50px] h-[50px]" />
                      </div>
                      <p className="text-xs text-gray-500 mt-2">1:1 原大</p>
                    </div>
                    <div className="text-center">
                      <div className="bg-white dark:bg-gray-800 shadow-md p-1 rounded inline-block">
                        <img src={processedUrl} alt="100x100" className="w-[100px] h-[100px] image-pixelated" />
                      </div>
                      <p className="text-xs text-gray-500 mt-2">200% 放大</p>
                    </div>
                  </div>
                  <button
                  ...
                  >
                    <Download className="w-4 h-4" /> 下载图标
                  </button>
                </>
              ) : (
